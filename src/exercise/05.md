...
Spread operator to simplify output
```js
...
Can use ternary conditions to make a little more elaboate and prevent unnecessary output
```js
const sizeClassName = size ? `box--${size}` : ''
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=david%40d2creativedesign.com).
