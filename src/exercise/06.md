...
`event.target` can be used to target input for onChange event.
`useState` to set the error
`useRef` to target the element instead of `getElementByID()` or `querySelector()`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=david%40d2creativedesign.com).
